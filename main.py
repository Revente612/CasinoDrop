...
# Инициализация бота
bot = TeleBot('5001368696:AAEtqufmeXROZAjA2IBBHJZDRe_iiPFPTmE/test', threaded=True, num_threads=5)
ADMINS = [5001448188, 2201285640]  # Замени на свои ID админов
...
# Подключение к БД
conn = sqlite3.connect('casino.db', check_same_thread=False, isolation_level=None, timeout=30)
cursor = conn.cursor()
...
def game_handler(call):
    ...
    if is_win:
        ...
        if game_type == 'dice':
            user_dice = random.randint(4, 6)
            bot_dice = random.randint(1, 3)
